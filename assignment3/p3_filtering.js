...
async function drawMap() {
  ...
  importanceFactorSliderInput = importanceFactorSliderArea
    .append("input")
    .attr("id", "value-importanceFactor")
    .attr("type", "number")
    .style("display", "block")
    .attr("value", d3.min(airports.map(airport => airport.importanceFactor)))
    .attr("min", d3.min(airports.map(airport => airport.importanceFactor)))
    .attr("max", d3.max(airports.map(airport => airport.importanceFactor)))
    .on("change", function() {
      console.log("importance factor input change");
      let inputValue = this.value;
      console.log("value: ", inputValue);
      importanceFactorSliderGenerator.silentValue(inputValue);
      // importanceFactorSliderGenerator.silentValue(inputValue / 100);

      let filteredAirports = airports
        .filter(d => d.coordinates != null)
        .filter(d => d.importanceFactor >= inputValue)
        .sort((a, b) => b.importanceFactor - a.importanceFactor);
  ...
  frequencySliderInput = frequencySliderArea
    .append("input")
    .attr("id", "value-frequency")
    .attr("type", "number")
    .style("display", "block")
    .attr("value", d3.min(linkData.map(link => link.value)))
    .attr("min", d3.min(linkData.map(link => link.value)))
    .attr("max", d3.max(linkData.map(link => link.value)))
    .on("change", function() {
      console.log("frequency input change");
      let inputValue = this.value;
      console.log("value: ", inputValue);
      frequencySliderGenerator.silentValue(inputValue);

      let filteredFlights = link.filter(link => link.value >= inputValue);
      let filteredAirportsIds = (() => {
        let filteredAirportsSet = new Set();
        filteredFlights.forEach(link => {
          filteredAirportsSet.push(link.source);
          filteredAirportsSet.push(link.target);
        });
        return filteredAirportsSet;
      })();

      let filteredAirports = airports.filter(airport =>
        filteredAirportsIds.includes(airport.id)
      );
